...
def train(
    ...
    model_dir = os.path.join(log_dir,"best_model_stage_1", "best_model_skip" + str(seed) + ".th")
    model_b = Resnet_Product_Of_Experts(num_classes=num_classes)
    model_b.to(device)
    model_b.load_state_dict(torch.load(model_dir)['state_dict'],strict=False)
    model_b.feature_weight = 1
    model_b.skip_weight = 0

    model_d_dir = os.path.join(log_dir,"best_model_stage_1", "best_model_skip" + str(seed) + ".th")
    model_d = Resnet_Skip_Model(num_classes=num_classes).to(device)
    model_d.load_state_dict(torch.load(model_d_dir)['state_dict'],strict=False)
    ...
    test_acc= evaluate(model_b, train_loader)
    eye_tsr = torch.eye(num_classes)
    print(test_acc[eye_tsr > 0.0].mean())
    print(test_acc[eye_tsr == 0.0].mean())
    ...
    for step in tqdm(range(main_num_steps)):
        ...
        loss_weight = (attr[:, bias_attr_idx] != attr[:, target_attr_idx]).long().detach()
        # print(loss_weight)
