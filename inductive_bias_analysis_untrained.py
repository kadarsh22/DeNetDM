...
def train(
    ...
    num_models_step = 2001

    # set all other train/ metrics to use this step
    wandb.define_metric("debias/*", step_metric="model_num")
    wandb.define_metric("bias/*", step_metric="model_num")
    ...
    for idx in range(2):
        ...
        for epoch in range(num_epochs):
            ...
            for _, data, attr in train_loader:
                ...
                logit = model(data, bias_weight=1, debias_weight=0)
        ...
        acc_shape, _, _ = evaluate_shape(model, valid_loader, debias_weight=0,
                                         bias_weight=1)
        print('idx :' + str(idx) + ' name : 3 layered MLP shape_acc : ' + str(acc_shape))
        acc_color, _, _ = evaluate_color(model, valid_loader, debias_weight=0,
                                         bias_weight=1)
        print('idx :' + str(idx) + ' name : 3 layered MLP color_acc : ' + str(acc_color))
