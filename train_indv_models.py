...
def train(
    ...
    #model.load_state_dict(torch.load('results/cmnist/ColoredMNIST-Skewed0.01-Severity4/stage1/2/gce_model_normalised.th'))
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch % main_log_freq) == 0:
            ...
            if (label == bias_attr).any().item():
                ...
                wandb.log({"loss/train_aligned": aligned_loss, "epoch": epoch})

            if (label != bias_attr).any().item():
                skewed_loss = loss_per_sample[label != bias_attr].mean()
                wandb.log({"loss/train_skewed": skewed_loss, "epoch": epoch})

        if (epoch % main_valid_freq) == 0:
            valid_accs, valid_aligned, valid_conflict = evaluate(model, valid_loader)
            wandb.log({"acc/valid": valid_accs, "epoch": epoch})
            wandb.log({"acc/valid_aligned": valid_aligned, "epoch": epoch})
            wandb.log({"acc/valid_skewed": valid_conflict, "epoch": epoch})
            
        debiased_model_path = os.path.join(save_path, 'erm_model_unbiased.th')
